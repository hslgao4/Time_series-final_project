...
model1 = sm.tsa.SARIMAX(train, order = (0, 0, 0), seasonal_order= (1,1,1,24))
...
train, test = train_test_split(diff_data, test_size=0.2, shuffle=False, random_state=6313)
model1 = sm.tsa.SARIMAX(train, order = (1, 0, 0), seasonal_order= (1,1,1,24))
model_fit_1 = model1.fit()
result_1 = model_fit_1.predict(start=0, end=len(train)-1)
...
### add na/nb 1
train, test = train_test_split(diff_data, test_size=0.2, shuffle=False, random_state=6313)
model1 = sm.tsa.SARIMAX(train, order = (2, 0, 1), seasonal_order= (1,1,1,24))
model_fit_1 = model1.fit()
result_1 = model_fit_1.predict(start=0, end=len(train)-1)
...
plot = cal_ACF(err_sarima_1, 300, "Residual")
ACF_PACF_Plot(err_sarima_1, lags=100)
ACF = sm.tsa.acf(err_sarima_1, nlags=80)
table_error = GPAC_table(ACF, J=12, K=12)
...
### non-seasonal diff
diff_data_24_1 = diff(diff_data_24, 1)
ACF1 = sm.tsa.acf(diff_data_24_1, nlags=200)
table1 = GPAC_table(ACF1, J=12, K=12)   # na=1, nb=0
ACF_PACF_Plot(diff_data_24_1, lags=200)

train, test = train_test_split(diff_data, test_size=0.2, shuffle=False, random_state=6313)
model1 = sm.tsa.SARIMAX(train, order = (2, 1, 1), seasonal_order= (1,1,1,24))
model_fit_1 = model1.fit()
result_1 = model_fit_1.predict(start=0, end=len(train)-1)
